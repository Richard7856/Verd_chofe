...
export function StepSummary({
  ...
  const respuestas = Object.values(draft.items)
  const okCount = respuestas.filter((r) => r.status === 'ok').length
  const fallas = items.filter((item) => draft.items[item.codigo]?.status === 'no_ok')

  const recorrido =
    draft.odometerStart != null && draft.odometerEnd != null
      ? draft.odometerEnd - draft.odometerStart
      : null

  return (
    <>
      <div className="space-y-3 p-4">
        <SectionTitle hint="Verificá la información antes de firmar.">
          Resumen del Turno
        </SectionTitle>

        {/* La entrada se envió al abrir el turno: acá sólo se consulta. */}
        <Card>
          <p className="mb-1 font-bold text-brand-600">Entrada</p>
          ...
          <p className="mt-2 text-xs text-body-soft">
            Registrada al abrir el turno. Ya no se puede modificar.
          </p>
        </Card>
        ...
        <Card>
          ...
          <div className="divide-y divide-gray-100">
            ...
            <Row label="Recorrido" value={recorrido != null ? km(recorrido) : '—'} />
            <Row label="Ruta" value={draft.shiftLabel || '—'} />
          </div>
        </Card>

        <Card>
          <p className="mb-1 font-bold text-brand-600">Check list de entrada</p>
          <div className="divide-y divide-gray-100">
            <div className="flex items-center justify-between py-2 text-sm">
              ...
              <span className="flex items-center gap-1.5 font-medium text-ink">
                ...
                <Icon
                  name={fallas.length === 0 ? 'checkCircle' : 'alert'}
                  size={16}
                  className={fallas.length === 0 ? 'text-brand-500' : 'text-accent-600'}
                />
              </span>
            </div>
            ...
            <div className="flex items-center justify-between py-2 text-sm">
              ...
              <span className="flex items-center gap-1.5 font-medium text-ink">
                {photoCount || slots.length} enviadas
                <Icon name="checkCircle" size={16} className="text-brand-500" />
              </span>
            </div>
